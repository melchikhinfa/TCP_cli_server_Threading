...
class UserRegistration:
    def __init__(self):
        ...
        self.conn = sl.connect(DB_PATH, check_same_thread=False, timeout=10)
    ...
    def userauth(self, name, password):
        ...
        with sl.connect(DB_PATH, check_same_thread=False, timeout=10) as conn:
            db_logger.db_log.info("Подключение к базе данных установлено.")
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM users WHERE name = ?", (name,))
            db_logger.db_log.info(f"Проверка наличия пользователя {name} в базе данных")
    ...
    def clear_table(self):
        with sl.connect(DB_PATH, check_same_thread=False) as conn:
            ...
            conn.commit()
            db_logger.db_log.info("Таблица users очищена. Соединение с базой данных закрыто.")
